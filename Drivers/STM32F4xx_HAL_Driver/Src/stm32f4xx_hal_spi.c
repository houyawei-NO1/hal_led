...
uint16_t temp666;uint16_t *temp666_p;//这两个是用于观察变量添加的
...
/**
  ...
  */
  ...
  HAL_StatusTypeDef HAL_SPI_TransmitReceive_16(SPI_HandleTypeDef *hspi,   uint16_t *pTxData, uint16_t *pRxData,
                                          uint16_t Size, uint32_t Timeout)
{
  uint16_t             initial_TxXferCount;
  uint32_t             tmp_mode;
  ...
  hspi->RxXferCount = Size;
  hspi->RxXferSize  = Size;
  hspi->pTxBuffPtr  = (const uint8_t *)pTxData;temp666_p=( uint16_t *)pTxData;
  hspi->TxXferCount = Size;
  hspi->TxXferSize  = Size;
  ...
  if (hspi->Init.DataSize == SPI_DATASIZE_16BIT)
  {
    ...
    {
      hspi->Instance->DR = *((const uint16_t *)hspi->pTxBuffPtr);
            hspi->Instance->DR = *((const uint16_t *)temp666_p);
            temp666= *((const uint16_t *)temp666_p);
      hspi->pTxBuffPtr += sizeof(uint16_t);
      hspi->TxXferCount--;
    ...
    while ((hspi->TxXferCount > 0U) || (hspi->RxXferCount > 0U))
    {
      ...
      {
        hspi->Instance->DR = *((const uint16_t *)hspi->pTxBuffPtr);
                temp666= *((const uint16_t *)hspi->pTxBuffPtr);
        hspi->pTxBuffPtr += sizeof(uint16_t);
        hspi->TxXferCount--;
        /* Next Data is a reception (Rx). Tx not allowed */
        txallowed = 0U;
