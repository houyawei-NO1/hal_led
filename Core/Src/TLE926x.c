...
SBC_ErrorCode sbc_write_reg(sbc_register_t sbc_register, uint8_t SBC_Val, uint16_t * returnval) {                      
	...
	uint16_t read_value = sbc_read_reg(sbc_register);
    errCode.flippedBitsMask = ((uint8_t)read_value) ^ SBC_Val;                             
...
/* Check if SBC is Mid-Range SBC */
	uint16_t value = sbc_read_reg(SBC_FAM_PROD_STAT);
    famprod = value;
...
//	uint16_t value1 = sbc_read_reg(SBC_WD_CTRL);
//	errCode = sbc_write_reg(SBC_WD_CTRL,0XFF, NULL);
//	uint16_t value2 = sbc_read_reg(SBC_WD_CTRL);
    /*soft reset*/
    sbc_write_reg_field (SBC_M_S_CTRL, MODE_MASK, SBC_RESET, 0x00); // M_S_CTRL(0x81)//SBC_RESET
    HAL_Delay(100); 
    ...
    while(initSequence[i][0] != 0x00U || initSequence[i][1] != 0x00U) {
     errCode = sbc_write_reg((sbc_register_t) initSequence[i][0], initSequence[i][1], NULL);
...
/* Check SBC in Normal Mode */
    ...
    registerReadOut = (uint8_t)sbc_read_reg(SBC_SWK_STAT);
        ...
        /* Empty */ 
            ...
            errCode.SBC_Register = SBC_SWK_STAT;
            errCode.expectedValue = 0x44U; // 0b01000100U -> 0x44U
